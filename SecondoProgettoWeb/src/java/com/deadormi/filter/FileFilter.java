...
public class FileFilter implements Filter {
    ...
    private static Logger log = Logger.getLogger(FileFilter.class);
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        String gruppo_id_string = null;
        gruppo_id_string = url.substring(url.indexOf("files/") + 6, url.lastIndexOf("/"));

        log.debug("gruppo " + gruppo_id_string);

        CrewController cc = new CrewController();
        Crew_UserController cu = new Crew_UserController();
        Crew crew = null;
        if (Parser.isNumeric(gruppo_id_string)) {
            try {
                Integer gruppo_id = Integer.parseInt(gruppo_id_string);
                crew = cc.find_crew_by_id(gruppo_id);


                if (crew == null || (crew.isCrew_private() && u != null && !cu.crew_belongs_to_user(crew.getId(), u.getId())) || (crew.isCrew_private() && u == null)) {
                    ((HttpServletResponse) response).sendRedirect("/SecondoProgettoWeb/GroupsServlet");
                } else {
